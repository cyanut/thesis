...
A support vector machine aims to find a hyperplane $\mathbf{w}_T\mathbf{x}+b=0$ which separates the target classes with maximum margin. Concretely, for features $\mathbf{x}$ and targets $y$, the support vector machine aim to maximize the minimum distance for each data points, transformed by a function $\phi$, to the classifying hyperplane:
\begin{equation*}
    \us{\mathbf{w},b}{argmax}\min_{i=1}^{n}\frac{y_i(\mathbf{w}^T\phi(\mathbf{x}_i)+b)}{|\mathbf{w}|}
\end{equation*}
 
Given that we can scale $\mathbf{w}$ and $b$ without changing the value of this term, we can scale $\mathbf{w}$ and $b$ such that the minimum distance to classifying plane $\min_{i=1}^n(y_i \cdot (\mathbf{w}^T\phi(\mathbf{x}_i + b))$ is equal to 1, therefore the original objective function becomes:
\begin{align*}
    &\us{\mathbf{w},b}{argmax}\min_{i=1}^{n}\frac{y_i(\mathbf{w}^T\phi(\mathbf{x}_i)+b)}{|\mathbf{w}|} \\
...
\end{align*}
under the constraints that the data points are correctly separated by the hyperplane with a distance of at least $1$:
\begin{equation*}
    y_i (\mathbf{w}^T\phi(\mathbf{x}_i) + b) - 1 \geq 0, \forall i
\end{equation*}
...
\begin{equation} \label{lagr}
    \mathcal{L}(\mathbf{w}, b, \mathbf{\lambda})=\frac{|\mathbf{w}|^2}{2} - \sum_{i=1}^n \lambda_i(y_i(\mathbf{w}^T\phi(\mathbf{x}_i) + b) - 1)
\end{equation}
where $\mathbf{\lambda}=(\lambda_1, \lambda_2, \ldots, \lambda_n)$, and minimize it by setting its partial derivatives regarding to $\mathbf{w}$ and $b$ to 0:
\begin{align}
    \nabla\mathbf{w} &= \mathbf{w} - \sum_{i=1}^n \lambda_i  y_i  \phi(\mathbf{x}_i) = 0 \label{dw}\\ 
    \frac{\partial{b}}{\partial{\mathcal{L}}} &= - \sum_{i=1}^n \lambda_i y_i = 0 \label{db}
\end{align}
Using (\ref{dw}) and (\ref{db}) to eliminate $\mathbf{w}$ and $b$ from the Lagrangian (\ref{lagr}), equivalently we aim to minimize the Lagrangian of $\mathbf{\lambda}$:
