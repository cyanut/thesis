...
\end{align*}
where the probability distribution $f_{x_n}(x_n|z_n)$ is the measurement model, which we approximate with a mixture of distributions of all features. The motion model $f_{z_n}(z_n|z_{n-1})$ is approximated with the distribution of speed at all directions. We used \num{2x10e4} particles for reliable tracking of the animals. The center of mass of all particles is used an an estimate of the animal's position $z_n$.
