...
Figure~\ref{f.ad.freezing} shows the percent of freezing during testing. Consistent with our hypothesis, Tg animals showed decreased freezing, and this was rescued by \tglu treatment. 
...
Figure~\ref{f.ad.activity} shows overall cell activity during training and testing. Consistent with previous reports in the literature \citep{verret12}, cells in Tg animals have increase overall cell activity. Interestingly, the \tglu administered before the training session corrected the cell activity in Tg to WT levels both during training and during testing. This suggests that either the rescuing effect is long-lasting, or successfully forming and recalling a memory corrects the cell activity to base level.
...
We then investigated that how \gls{wt} and \gls{tg} cells encodes freezing. First we looked at cells individually, and calculated the mutual information between cell firing and freezing \citep{skaggs93}. This measurement reflects how much prediction power a cell has for freezing in a period of time. Again, cells in Tg animals have less freezing information, and this effect is partially rescued by \tglu treatment (Figure~\ref{f.ad.freeze_info}). This result suggests that, not only do the \gls{tg} animals freeze less, but when they are freezing, the information is not well encoded in hippocampus \gls{ca1}.
\begin{figure}[h]
