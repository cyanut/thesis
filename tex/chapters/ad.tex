...
Using particle filter, we approximate the poosterior distribution $f_{z_n}(z_n|X_n)$ with $L$ particles at $\{a^i\}_{i=1}^n$ with weights $\{w^i\}_{i=1}^n$, such that the posterior distribution can be approximated as a mixture of Dirac distribution:
\begin{equation} \label{zn_appox}
...
\end{align*}
where the probability distribution $f_{x_n}(x_n|z_n)$ is the measurement model, which we approximate with a mixture of distributions of all features. The motion model $f_{z_n}(z_n|z_{n-1})$ is approximated with the distribution of speed at all directions. Conbined with (\ref{zn_approx}), we can find $\{a_n^i\}$ by sampling from the mixture of pulses $\sum_{i=1}^Lw_{n-1}^if_{z_n}(z_n|a_{n-1}^i)$, and calculate $w_n^i = f_{x_n}(x_n|a_n^i)$. The center of mass of all particles is used as an estimate of the animal's position $z_n$. This is then repeated for every frame.
