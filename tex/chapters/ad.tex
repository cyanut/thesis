...
\begin{equation*}
    \us{\mathbf{w},b}{argmax}\min_{i=1}^{n}\frac{y_i\cdot(\mathbf{w}^T\phi(\mathbf{x}_i)+b)}{|\mathbf{w}|}
\end{equation*}
...
\begin{align*}
    &\us{\mathbf{w},b}{argmax}\min_{i=1}^{n}\frac{y_i\cdot(\mathbf{w}^T\phi(\mathbf{x}_i)+b)}{|\mathbf{w}|} \\
    =&\us{\mathbf{w},b}{argmax}\frac{1}{|\mathbf{w}|}\min_{i=1}^{n}(y_i\cdot(\mathbf{w}^T\phi(\mathbf{x}_i)+b)) \\
    =&\us{\mathbf{w},b}{argmax}\frac{1}{|\mathbf{w}|}
\end{align*}
under the constraints that the data points are correctly separated by the hyperplane with a distance of at least $1$:
\begin{equation*}
    y_i \cdot (\mathbf{w}^T\phi(\mathbf{x}_i) + b) - 1 \geq 0, \forall i
\end{equation*}
