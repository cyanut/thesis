...
The mutual information measurement between two random variables represents the degree of relatedness between them. We estimated the mutual information between the calcium trace for each cell $C$, a continuous random variable and the freezing state of the animal $F$, a discrete random variable. Specificially, we used the \gls{kl} twice to estimate the entropy of calcium trace $H(C)$ and the conditional entropy $H(C|F)$ \citep{ross14, \todo{cite brian ross, kozachenko}}, and calculated the mutual information as:
